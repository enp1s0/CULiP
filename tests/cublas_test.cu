...
template <class T> struct get_real_type {
  using type = void;
};
template <> struct get_real_type<cuComplex> {
  using type = float;
};
template <> struct get_real_type<cuDoubleComplex> {
  using type = double;
};
...
#define GEMM_OP_GEMM(short_type, type)                                         \
  template <>                                                                  \
  cublasStatus_t gemm<type, op_gemm>(                                          \
      ...
      type *C, int ldc) {                                                      \
    return cublas##short_type##gemm(handle, transa, transb, m, n, k, alpha, A, \
...
#define GEMM_OP_GEMM_EX(cuda_data_type, type)                                  \
  template <>                                                                  \
  cublasStatus_t gemm<type, op_gemmEx>(                                        \
      cublasHandle_t handle, cublasOperation_t transa,                         \
      cublasOperation_t transb, int m, int n, int k, const type *alpha,        \
      const type *A, int lda, const type *B, int ldb, const type *beta,        \
      type *C, int ldc) {                                                      \
...
#define GEMM_BATCHED_OP_GEMM(short_type, type)                                 \
  template <>                                                                  \
  cublasStatus_t gemm_batched<type, op_gemm>(                                  \
      cublasHandle_t handle, cublasOperation_t transa,                         \
      cublasOperation_t transb, int m, int n, int k, const type *alpha,        \
      const type *A[], int lda, const type *B[], int ldb, const type *beta,    \
      type *C[], int ldc, int batchCount) {                                    \
    return cublas##short_type##gemmBatched(handle, transa, transb, m, n, k,    \
...
#define GEMM_BATCHED_OP_GEMMEX(cuda_data_type, type)                           \
  template <>                                                                  \
  cublasStatus_t gemm_batched<type, op_gemmEx>(                                \
      cublasHandle_t handle, cublasOperation_t transa,                         \
      cublasOperation_t transb, int m, int n, int k, const type *alpha,        \
      const type *A[], int lda, const type *B[], int ldb, const type *beta,    \
      type *C[], int ldc, int batchCount) {                                    \
    return cublasGemmBatchedEx(                                                \
        ...
        reinterpret_cast<const void **>(A), cuda_data_type, lda,               \
        reinterpret_cast<const void **>(B), cuda_data_type, ldb, beta,         \
        reinterpret_cast<void **>(C), cuda_data_type, ldc, batchCount,         \
        cuda_data_type, CUBLAS_GEMM_DEFAULT);                                  \
  }
...
template <class T, class Op>
cublasStatus_t
gemm_strided_batched(cublasHandle_t handle, cublasOperation_t transa,
                     cublasOperation_t transb, int m, int n, int k,
                     const T *alpha, const T *A, int lda, long long int strideA,
                     const T *B, int ldb, long long int strideB, const T *beta,
                     T *C, int ldc, long long int strideC, int batchCount);
// -----------------------------------------------------
// op_gemm
// -----------------------------------------------------
#define GEMM_STRIDED_BATCHED_OP_GEMM(short_type, type)                         \
  template <>                                                                  \
  cublasStatus_t gemm_strided_batched<type, op_gemm>(                          \
      cublasHandle_t handle, cublasOperation_t transa,                         \
      cublasOperation_t transb, int m, int n, int k, const type *alpha,        \
      const type *A, int lda, long long int strideA, const type *B, int ldb,   \
      long long int strideB, const type *beta, type *C, int ldc,               \
      long long int strideC, int batchCount) {                                 \
    return cublas##short_type##gemmStridedBatched(                             \
...
#define GEMM_STRIDED_BATCHED_OP_GEMMEX(cuda_data_type, type)                   \
  template <>                                                                  \
  cublasStatus_t gemm_strided_batched<type, op_gemmEx>(                        \
      cublasHandle_t handle, cublasOperation_t transa,                         \
      cublasOperation_t transb, int m, int n, int k, const type *alpha,        \
      const type *A, int lda, long long int strideA, const type *B, int ldb,   \
      long long int strideB, const type *beta, type *C, int ldc,               \
    ...
    return cublasGemmStridedBatchedEx(                                         \
        handle, transa, transb, m, n, k, alpha,                                \
        reinterpret_cast<const void *>(A), cuda_data_type, lda, strideA,       \
        reinterpret_cast<const void *>(B), cuda_data_type, ldb, strideB, beta, \
        reinterpret_cast<void *>(C), cuda_data_type, ldc, strideC, batchCount, \
        cuda_data_type, CUBLAS_GEMM_DEFAULT);                                  \
  }
...
cublasStatus_t gemv(cublasHandle_t handle, cublasOperation_t trans, int m,
                    ...
                    int incx, const T *beta, T *y, int incy);
#define GEMM_OP_GEMV(short_type, type)                                         \
  template <>                                                                  \
  cublasStatus_t gemv<type>(cublasHandle_t handle, cublasOperation_t trans,    \
                            int m, int n, const type *alpha, const type *A,    \
                            int lda, const type *x, int incx,                  \
                            const type *beta, type *y, int incy) {             \
    return cublas##short_type##gemv(handle, trans, m, n, alpha, A, lda, x,     \
                                    incx, beta, y, incy);                      \
  }
...
// -------------
template <class T>
cublasStatus_t gbmv(cublasHandle_t handle, cublasOperation_t trans, int m,
                    int n, int kl, int ku, const T *alpha, const T *A, int lda,
...
#define GEMM_OP_GBMV(short_type, type)                                         \
  ...
  cublasStatus_t gbmv<type>(cublasHandle_t handle, cublasOperation_t trans,    \
                            int m, int n, int kl, int ku, const type *alpha,   \
                            const type *A, int lda, const type *x, int incx,   \
                            const type *beta, type *y, int incy) {             \
    return cublas##short_type##gbmv(handle, trans, m, n, kl, ku, alpha, A,     \
...
// -------------
template <class T>
cublasStatus_t ger(cublasHandle_t handle, int m, int n, const T *alpha,
                   const T *x, int incx, const T *y, int incy, T *A, int lda);
#define GEMM_OP_GER(short_type, type)                                          \
  template <>                                                                  \
  cublasStatus_t ger<type>(cublasHandle_t handle, int m, int n,                \
                           const type *alpha, const type *x, int incx,         \
                           const type *y, int incy, type *A, int lda) {        \
    return cublas##short_type##ger(handle, m, n, alpha, x, incx, y, incy, A,   \
                                   lda);                                       \
  }
GEMM_OP_GER(S, float);
...
template <class T>
cublasStatus_t gerc(cublasHandle_t handle, int m, int n, const T *alpha,
                    const T *x, int incx, const T *y, int incy, T *A, int lda);
#define GEMM_OP_GERC(short_type, type)                                         \
  template <>                                                                  \
  cublasStatus_t gerc<type>(cublasHandle_t handle, int m, int n,               \
                            const type *alpha, const type *x, int incx,        \
                            const type *y, int incy, type *A, int lda) {       \
    return cublas##short_type##gerc(handle, m, n, alpha, x, incx, y, incy, A,  \
                                    lda);                                      \
  }
...
template <class T>
cublasStatus_t geru(cublasHandle_t handle, int m, int n, const T *alpha,
                    const T *x, int incx, const T *y, int incy, T *A, int lda);
#define GEMM_OP_GERU(short_type, type)                                         \
  template <>                                                                  \
  cublasStatus_t geru<type>(cublasHandle_t handle, int m, int n,               \
                            const type *alpha, const type *x, int incx,        \
                            const type *y, int incy, type *A, int lda) {       \
    return cublas##short_type##geru(handle, m, n, alpha, x, incx, y, incy, A,  \
                                    lda);                                      \
  }
...
// -------------
template <class T>
cublasStatus_t sbmv(cublasHandle_t handle, cublasFillMode_t uplo, int m, int n,
...
#define GEMM_OP_SBMV(short_type, type)                                         \
  ...
  cublasStatus_t sbmv<type>(cublasHandle_t handle, cublasFillMode_t uplo,      \
                            ...
                            int lda, const type *x, int incx,                  \
                            const type *beta, type *y, int incy) {             \
    return cublas##short_type##sbmv(handle, uplo, m, n, alpha, A, lda, x,      \
                                    incx, beta, y, incy);                      \
  }
...
// -------------
template <class T>
cublasStatus_t spmv(cublasHandle_t handle, cublasFillMode_t uplo, int m,
                    const T *alpha, const T *A, const T *x, int incx,
                    const T *beta, T *y, int incy);
#define GEMM_OP_SPMV(short_type, type)                                         \
  template <>                                                                  \
  cublasStatus_t spmv<type>(cublasHandle_t handle, cublasFillMode_t uplo,      \
                            int m, const type *alpha, const type *A,           \
                            const type *x, int incx, const type *beta,         \
                            type *y, int incy) {                               \
    return cublas##short_type##spmv(handle, uplo, m, alpha, A, x, incx, beta,  \
                                    y, incy);                                  \
  }
...
// -------------
template <class T>
cublasStatus_t syrk(cublasHandle_t handle, cublasFillMode_t uplo,
...
#define GEMM_OP_SYRK(short_type, type)                                         \
  ...
  cublasStatus_t syrk<type>(cublasHandle_t handle, cublasFillMode_t uplo,      \
    ...
    return cublas##short_type##syrk(handle, uplo, trans, n, k, alpha, A, lda,  \
                                    beta, C, ldc);                             \
  }
...
// -------------
template <class T>
cublasStatus_t syr2k(cublasHandle_t handle, cublasFillMode_t uplo,
                     cublasOperation_t trans, int n, int k, const T *alpha,
                     const T *A, int lda, const T *B, int ldb, const T *beta,
                     T *C, int ldc);
#define GEMM_OP_SYR2K(short_type, type)                                        \
  template <>                                                                  \
  cublasStatus_t syr2k<type>(                                                  \
      cublasHandle_t handle, cublasFillMode_t uplo, cublasOperation_t trans,   \
      int n, int k, const type *alpha, const type *A, int lda, const type *B,  \
      int ldb, const type *beta, type *C, int ldc) {                           \
    return cublas##short_type##syr2k(handle, uplo, trans, n, k, alpha, A, lda, \
                                     B, ldb, beta, C, ldc);                    \
  }
...
// -------------
template <class T>
cublasStatus_t syrkx(cublasHandle_t handle, cublasFillMode_t uplo,
                     cublasOperation_t trans, int n, int k, const T *alpha,
                     const T *A, int lda, const T *B, int ldb, const T *beta,
                     T *C, int ldc);
#define GEMM_OP_SYRKX(short_type, type)                                        \
  template <>                                                                  \
  cublasStatus_t syrkx<type>(                                                  \
      cublasHandle_t handle, cublasFillMode_t uplo, cublasOperation_t trans,   \
      int n, int k, const type *alpha, const type *A, int lda, const type *B,  \
      int ldb, const type *beta, type *C, int ldc) {                           \
    return cublas##short_type##syrkx(handle, uplo, trans, n, k, alpha, A, lda, \
                                     B, ldb, beta, C, ldc);                    \
  }
...
#define GEMM_OP_SYMM(short_type, type)                                         \
  template <>                                                                  \
  cublasStatus_t symm<type>(                                                   \
      cublasHandle_t handle, cublasSideMode_t side, cublasFillMode_t uplo,     \
      int m, int n, const type *alpha, const type *A, int lda, const type *B,  \
      int ldb, const type *beta, type *C, int ldc) {                           \
    return cublas##short_type##symm(handle, side, uplo, m, n, alpha, A, lda,   \
                                    B, ldb, beta, C, ldc);                     \
  }
...
// -------------
template <class T>
cublasStatus_t trmm(cublasHandle_t handle, cublasSideMode_t side,
...
#define GEMM_OP_TRMM(short_type, type)                                         \
  template <>                                                                  \
  cublasStatus_t trmm<type>(cublasHandle_t handle, cublasSideMode_t side,      \
                            cublasFillMode_t uplo, cublasOperation_t trans,    \
                            cublasDiagType_t diag, int m, int n,               \
                            const type *alpha, const type *A, int lda,         \
                            const type *B, int ldb, type *C, int ldc) {        \
    return cublas##short_type##trmm(handle, side, uplo, trans, diag, m, n,     \
...
// -------------
template <class T>
cublasStatus_t trsm(cublasHandle_t handle, cublasSideMode_t side,
                    cublasFillMode_t uplo, cublasOperation_t trans,
                    cublasDiagType_t diag, int m, int n, const T *alpha,
                    const T *A, int lda, T *B, int ldb);
#define GEMM_OP_TRSM(short_type, type)                                         \
  template <>                                                                  \
  cublasStatus_t trsm<type>(                                                   \
      cublasHandle_t handle, cublasSideMode_t side, cublasFillMode_t uplo,     \
      cublasOperation_t trans, cublasDiagType_t diag, int m, int n,            \
      const type *alpha, const type *A, int lda, type *B, int ldb) {           \
    return cublas##short_type##trsm(handle, side, uplo, trans, diag, m, n,     \
                                    alpha, A, lda, B, ldb);                    \
  }
...
// -------------
template <class T>
cublasStatus_t trsm_batched(cublasHandle_t handle, cublasSideMode_t side,
                            cublasFillMode_t uplo, cublasOperation_t trans,
                            cublasDiagType_t diag, int m, int n, const T *alpha,
...
#define GEMM_OP_TRSM_BATCHED(short_type, type)                                 \
  template <>                                                                  \
  cublasStatus_t trsm_batched<type>(                                           \
      cublasHandle_t handle, cublasSideMode_t side, cublasFillMode_t uplo,     \
      cublasOperation_t trans, cublasDiagType_t diag, int m, int n,            \
      const type *alpha, const type *const A[], int lda, type *const B[],      \
      int ldb, int batchCount) {                                               \
    return cublas##short_type##trsmBatched(handle, side, uplo, trans, diag, m, \
                                           n, alpha, A, lda, B, ldb,           \
                                           batchCount);                        \
  }
...
// -----------------------------------------------------
template <class T>
cublasStatus_t hemm(cublasHandle_t handle, cublasSideMode_t side,
                    cublasFillMode_t uplo, int m, int n, const T *alpha,
                    const T *A, int lda, const T *B, int ldb, const T *beta,
                    T *C, int ldc);
#define GEMM_OP_HEMM(short_type, type)                                         \
  template <>                                                                  \
  cublasStatus_t hemm<type>(                                                   \
      cublasHandle_t handle, cublasSideMode_t side, cublasFillMode_t uplo,     \
      int m, int n, const type *alpha, const type *A, int lda, const type *B,  \
      int ldb, const type *beta, type *C, int ldc) {                           \
    return cublas##short_type##hemm(handle, side, uplo, m, n, alpha, A, lda,   \
                                    B, ldb, beta, C, ldc);                     \
  }
...
cublasStatus_t herk(cublasHandle_t handle, cublasFillMode_t uplo,
                    cublasOperation_t trans, int m, int n, const RT *alpha,
                    const T *A, int lda, const RT *beta, T *C, int ldc);
#define GEMM_OP_HERK(short_type, type, real_type)                              \
  template <>                                                                  \
  cublasStatus_t herk<type, real_type>(                                        \
      cublasHandle_t handle, cublasFillMode_t uplo, cublasOperation_t trans,   \
      int m, int n, const real_type *alpha, const type *A, int lda,            \
      const real_type *beta, type *C, int ldc) {                               \
    return cublas##short_type##herk(handle, uplo, trans, m, n, alpha, A, lda,  \
                                    beta, C, ldc);                             \
  }
...
// -----------------------------------------------------
template <class T, class RT>
cublasStatus_t her2k(cublasHandle_t handle, cublasFillMode_t uplo,
                     cublasOperation_t trans, int m, int n, const T *alpha,
                     const T *A, int lda, const T *B, int ldb, const RT *beta,
                     T *C, int ldc);
#define GEMM_OP_HER2K(short_type, type, real_type)                             \
  template <>                                                                  \
  cublasStatus_t her2k<type, real_type>(                                       \
      cublasHandle_t handle, cublasFillMode_t uplo, cublasOperation_t trans,   \
      int m, int n, const type *alpha, const type *A, int lda, const type *B,  \
      int ldb, const real_type *beta, type *C, int ldc) {                      \
    return cublas##short_type##her2k(handle, uplo, trans, m, n, alpha, A, lda, \
                                     B, ldb, beta, C, ldc);                    \
  }
...
// -----------------------------------------------------
template <class T, class RT>
cublasStatus_t herkx(cublasHandle_t handle, cublasFillMode_t uplo,
                     cublasOperation_t trans, int m, int n, const T *alpha,
                     const T *A, int lda, const T *B, int ldb, const RT *beta,
                     T *C, int ldc);
#define GEMM_OP_HERKX(short_type, type, real_type)                             \
  template <>                                                                  \
  cublasStatus_t herkx<type, real_type>(                                       \
      cublasHandle_t handle, cublasFillMode_t uplo, cublasOperation_t trans,   \
      int m, int n, const type *alpha, const type *A, int lda, const type *B,  \
      int ldb, const real_type *beta, type *C, int ldc) {                      \
    return cublas##short_type##herkx(handle, uplo, trans, m, n, alpha, A, lda, \
                                     B, ldb, beta, C, ldc);                    \
  }
...
#define GEMM_OP_GEMM3M(short_type, type)                                       \
  ...
  cublasStatus_t gemm3m<type>(cublasHandle_t handle, cublasOperation_t transa, \
                              cublasOperation_t transb, int m, int n, int k,   \
                              const type *alpha, const type *A, int lda,       \
                              const type *B, int ldb, const type *beta,        \
                              type *C, int ldc) {                              \
    return cublas##short_type##gemm3m(handle, transa, transb, m, n, k, alpha,  \
                                      A, lda, B, ldb, beta, C, ldc);           \
  }
GEMM_OP_GEMM3M(C, cuComplex);
GEMM_OP_GEMM3M(Z, cuDoubleComplex);

template <class T> T convert(const double a) { return static_cast<T>(a); }
template <> cuComplex convert<cuComplex>(const double a) {
  return make_float2(a, 0);
}
template <> cuDoubleComplex convert<cuDoubleComplex>(const double a) {
  return make_double2(a, 0);
}

template <class T, class Op> void gemm_test() {
  ...
  gemm<T, Op>(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, n, n, n, &alpha, mat_a,
              n, mat_b, n, &beta, mat_c, n);
...
template <class T, class Op> void gemm_batched_test() {
  const int n = 1lu << 7;
  ...
  gemm_batched<T, Op>(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, n, n, n, &alpha,
                      (const T **)mat_a_array, n, (const T **)mat_b_array, n,
                      &beta, mat_c_array, n, batch_size);
...
template <class T, class Op> void gemm_strided_batched_test() {
  const int n = 1lu << 7;
  const int batch_size = 1u << 10;
  const auto alpha = convert<T>(1);
  const auto beta = convert<T>(0);
  ...
  gemm_strided_batched<T, Op>(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, n, n, n,
                              &alpha, mat_a, n, n * n, mat_b, n, n * n, &beta,
                              mat_c, n, n * n, batch_size);
...
template <class T> void gemv_test() {
  const std::size_t n = 1lu << 10;
  const auto alpha = convert<T>(1);
  const auto beta = convert<T>(0);
  ...
  gemv<T>(cublas_handle, CUBLAS_OP_N, n, n, &alpha, mat_a, n, vec_x, 1, &beta,
          vec_y, 1);
...
template <class T> void gbmv_test() {
  const std::size_t n = 1lu << 10;
  const auto alpha = convert<T>(1);
  const auto beta = convert<T>(0);
  ...
  gbmv<T>(cublas_handle, CUBLAS_OP_N, n, n, n / 10, n / 10, &alpha, mat_a, n,
          vec_x, 1, &beta, vec_y, 1);
...
template <class T> void sbmv_test() {
  const std::size_t n = 1lu << 10;
  const auto alpha = convert<T>(1);
  const auto beta = convert<T>(0);
  ...
  sbmv<T>(cublas_handle, CUBLAS_FILL_MODE_UPPER, n, n, &alpha, mat_a, n + 1,
          vec_x, 1, &beta, vec_y, 1);
...
template <class T> void spmv_test() {
  const std::size_t n = 1lu << 10;
  const auto alpha = convert<T>(1);
  const auto beta = convert<T>(0);
  ...
  spmv<T>(cublas_handle, CUBLAS_FILL_MODE_UPPER, n, &alpha, mat_a, vec_x, 1,
          &beta, vec_y, 1);
...
template <class T> void ger_test() {
  const std::size_t n = 1lu << 10;
  const auto alpha = convert<T>(1);
  ...
  ger<T>(cublas_handle, n, n, &alpha, vec_x, 1, vec_y, 1, mat_a, n);
...
template <class T> void gerc_test() {
  const std::size_t n = 1lu << 10;
  const auto alpha = convert<T>(1);
  ...
  gerc<T>(cublas_handle, n, n, &alpha, vec_x, 1, vec_y, 1, mat_a, n);
...
template <class T> void geru_test() {
  const std::size_t n = 1lu << 10;
  const auto alpha = convert<T>(1);
  ...
  geru<T>(cublas_handle, n, n, &alpha, vec_x, 1, vec_y, 1, mat_a, n);
...
template <class T> void syrk_test() {
  const std::size_t n = 1lu << 10;
  const auto alpha = convert<T>(1);
  const auto beta = convert<T>(0);
  ...
  syrk<T>(cublas_handle, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N, n, n, &alpha,
          mat_a, n, &beta, mat_c, n);
...
template <class T> void syr2k_test() {
  const std::size_t n = 1lu << 10;
  const auto alpha = convert<T>(1);
  const auto beta = convert<T>(0);
  ...
  syr2k<T>(cublas_handle, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N, n, n, &alpha,
           mat_a, n, mat_b, n, &beta, mat_c, n);
...
template <class T> void syrkx_test() {
  const std::size_t n = 1lu << 10;
  const auto alpha = convert<T>(1);
  const auto beta = convert<T>(0);
  ...
  syrkx<T>(cublas_handle, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N, n, n, &alpha,
           mat_a, n, mat_b, n, &beta, mat_c, n);
...
template <class T> void symm_test() {
  const std::size_t n = 1lu << 10;
  const auto alpha = convert<T>(1);
  const auto beta = convert<T>(0);
  ...
  symm<T>(cublas_handle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_LOWER, n, n, &alpha,
          mat_a, n, mat_b, n, &beta, mat_c, n);
...
template <class T> void trmm_test() {
  const std::size_t n = 1lu << 10;
  const auto alpha = convert<T>(1);
  ...
  trmm<T>(cublas_handle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N,
          CUBLAS_DIAG_NON_UNIT, n, n, &alpha, mat_a, n, mat_b, n, mat_c, n);
...
template <class T> void trsm_test() {
  const std::size_t n = 1lu << 10;
  const auto alpha = convert<T>(1);
  ...
  trsm<T>(cublas_handle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N,
          CUBLAS_DIAG_NON_UNIT, n, n, &alpha, mat_a, n, mat_b, n);
...
template <class T> void trsm_batched_test() {
  ...
  trsm_batched<T>(cublas_handle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_LOWER,
                  CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT, n, n, &alpha, mat_a_array,
                  n, mat_b_array, n, batch_size);
...
template <class T> void hemm_test() {
  const std::size_t n = 1lu << 10;
  const auto alpha = convert<T>(1);
  const auto beta = convert<T>(0);
  ...
  hemm<T>(cublas_handle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_LOWER, n, n, &alpha,
          mat_a, n, mat_b, n, &beta, mat_c, n);
...
template <class T> void herk_test() {
  ...
  herk<T>(cublas_handle, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N, n, n, &alpha,
          mat_a, n, &beta, mat_c, n);
...
template <class T> void her2k_test() {
  using real_type = typename get_real_type<T>::type;
  const std::size_t n = 1lu << 10;
  const auto alpha = convert<T>(1);
  const auto beta = convert<real_type>(0);
  ...
  her2k<T>(cublas_handle, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N, n, n, &alpha,
           mat_a, n, mat_b, n, &beta, mat_c, n);
...
template <class T> void herkx_test() {
  using real_type = typename get_real_type<T>::type;
  const std::size_t n = 1lu << 10;
  const auto alpha = convert<T>(1);
  const auto beta = convert<real_type>(0);
  ...
  herkx<T>(cublas_handle, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N, n, n, &alpha,
           mat_a, n, mat_b, n, &beta, mat_c, n);
...
template <class T> void gemm3m_test() {
  const std::size_t n = 1lu << 10;
  const auto alpha = convert<T>(1);
  const auto beta = convert<T>(0);
  ...
  gemm3m<T>(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, n, n, n, &alpha, mat_a, n,
            mat_b, n, &beta, mat_c, n);
