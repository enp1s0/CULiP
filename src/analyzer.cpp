...
struct aggregation_t {
  ...
  void add(const std::size_t time) {
    time_sum += time;
  ...
  aggregation_t(const std::string entry_name)
      : entry_name(entry_name), time_sum(0), time_max(0),
        time_min(std::numeric_limits<std::size_t>::max()), count(0) {}
};

void add_to_aggregation_list(std::vector<aggregation_t> &aggregation_list,
                             const std::string &entry_name,
                             const std::size_t elapsed_time) {
  // If the function has already pushed to aggregation list, just add elapsed
  // time to it
...
void add_to_detail_aggregation_list(
    std::vector<std::tuple<std::string, std::vector<aggregation_t>>>
        &detail_aggregation_list,
    const std::string &category_name, const std::string &entry_name,
    const std::size_t elapsed_time) {
  for (auto &aggregation : detail_aggregation_list) {
    if (std::get<0>(aggregation) == category_name) {
      add_to_aggregation_list(std::get<1>(aggregation), entry_name,
                              elapsed_time);
      return;
    }
  }
  ...
  detail_aggregation_list.push_back(std::make_tuple(category_name, new_list));
}
...
int main(int argc, char **argv) {
  ...
  std::vector<
      std::tuple<std::string /*category name*/, std::vector<aggregation_t>>>
      detail_aggregation_list;

  // For find CULiP result lines
  std::regex CULiP_result_regex(std::string(R"(\[)") + CULIP_RESULT_PREFIX +
                                R"(\]\[([a-zA-Z0-9_]+)-(.*)\] ([0-9]+)ns$)");
  ...
  while (std::getline(std::cin, line)) {
    if (std::regex_search(line, CULiP_smatch, CULiP_result_regex)) {
      ...
      // Add to list
      add_to_aggregation_list(category_aggregation_list, function_name,
                              elapsed_time);
      add_to_detail_aggregation_list(detail_aggregation_list, function_name,
                                     function_params, elapsed_time);

      // Update max name length
      max_category_name_length = std::max<std::size_t>(max_category_name_length,
                                                       function_name.length());
      max_detail_name_length = std::max<std::size_t>(max_detail_name_length,
  ...
  const auto compare_aggregation_t = [](const aggregation_t &a,
                                        const aggregation_t &b) {
    return a.time_sum > b.time_sum;
  };
  std::sort(category_aggregation_list.begin(), category_aggregation_list.end(),
            compare_aggregation_t);
  ...
  for (const auto &category_aggregation : category_aggregation_list) {
    ...
    std::printf("- %*s : [%lu ns; %e s;%6.2f%%]\n",
                static_cast<int>(max_category_name_length),
                category_name.c_str(), category_time, category_time * 1e-9,
                static_cast<double>(category_time) / time_sum * 100);

    auto detail_aggregation =
        get_detail_aggregation(detail_aggregation_list, category_name);
    std::sort(detail_aggregation.begin(), detail_aggregation.end(),
              compare_aggregation_t);

    std::printf("  %*s %8s %21s %12s %12s %12s\n",
                static_cast<int>(max_detail_name_length), "params", "count",
                "sum", "avg", "max", "min");
    ...
    for (const auto &da : detail_aggregation) {
      ...
      std::printf("  %*s %8lu %10.3fms(%6.2f%%) %10.3fms %10.3fms %10.3fms\n",
                  ...
                  da.entry_name.c_str(), da.count, da.time_sum * 1e-6,
                  static_cast<double>(da.time_sum) / category_time * 100,
                  da.time_sum * 1e-6 / da.count, da.time_max * 1e-6,
                  da.time_min * 1e-6);
    }
  }
