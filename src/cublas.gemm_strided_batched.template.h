cublasStatus_t CULIP_FUNC_NAME(cublasHandle_t handle, cublasOperation_t transa,
                               ...
                               long long int strideB, const CULIP_TYPE *beta,
                               CULIP_TYPE *C, int ldc, long long int strideC,
                               int batchCount) {
#ifdef __CUDA_ARCH__
    ...
    try {
      ...
      for (std::uint32_t i = 0; i < batchCount; i++) {
        mtk::cu_cutoff::cutoff_small_abs_values(
            const_cast<CULIP_TYPE *>(A + i * strideA),
            (transa == CUBLAS_OP_N ? m : k), (transa == CUBLAS_OP_N ? k : m),
            lda, threshold, cuda_stream);
        mtk::cu_cutoff::cutoff_small_abs_values(
            const_cast<CULIP_TYPE *>(B + i * strideB),
            (transb == CUBLAS_OP_N ? k : n), (transb == CUBLAS_OP_N ? n : k),
            ldb, threshold, cuda_stream);
      }
  ...
  cublasStatus_t (*cublas_lib_func)(
      ...
      const CULIP_TYPE *const, const CULIP_TYPE *, int, long long int,
      const CULIP_TYPE *, int, long long int, const CULIP_TYPE *const,
      CULIP_TYPE *, int, long long int, int);
  ...
  if (exp_stats_flag) {
    ...
    for (std::uint32_t i = 0; i < batchCount; i++) {
      a_stats.stats += mtk::cu_exp_statistics::take_matrix_statistics(
          A + i * strideA, (transa == CUBLAS_OP_N ? m : k),
          (transa == CUBLAS_OP_N ? k : m), lda, cuda_stream);
      b_stats.stats += mtk::cu_exp_statistics::take_matrix_statistics(
          B + i * strideB, (transb == CUBLAS_OP_N ? k : n),
          (transb == CUBLAS_OP_N ? n : k), ldb, cuda_stream);
    }
