cublasStatus_t CULIP_FUNC_NAME(cublasHandle_t handle, cublasOperation_t trans,
                               int m, int n, const CULIP_TYPE *alpha,
                               const CULIP_TYPE *A, int lda,
                               const CULIP_TYPE *x, int incx,
                               const CULIP_TYPE *beta, CULIP_TYPE *y,
                               int incy) {
#ifdef __CUDA_ARCH__
  ...
  cublasStatus_t (*cublas_lib_func)(cublasHandle_t, cublasOperation_t, int, int,
                                    const CULIP_TYPE *, const CULIP_TYPE *, int,
  ...
  *(void **)(&cublas_lib_func) = CULiP_get_function_pointer(
  ...
  if (profiling_flag) {
    ...
    snprintf(profile_result.function_name,
             profile_result.function_name_length - 1, "%s-%s-m%d-n%d", __func__,
             CULiP_get_cublasOperation_t_string(trans), m, n);
