cublasStatus_t CULIP_FUNC_NAME(cublasHandle_t handle,
	...
	*(void**)(&cublas_lib_func) = CULiP_get_function_pointer(CULIP_CUBLAS_LIBRARY_NAME, CULIP_CUBLAS_ENV_NAME, __func__, &CULiP_cublas_lib_handle_cache);
	...
	if (profiling_flag) {
		...
		snprintf(profile_result.function_name, profile_result.function_name_length - 1, "%s-%s-%s-%s-%s-m%d-n%d-batchCount%d",
				__func__,
	...
	// Call the function
	const cublasStatus_t result = (*cublas_lib_func)(handle, side, uplo, trans, diag, m, n, alpha, A, lda, B, ldb, batchCount);
