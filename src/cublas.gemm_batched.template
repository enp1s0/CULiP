cublasStatus_t CULIP_FUNC_NAME (cublasHandle_t handle,
		...
		const CULIP_TYPE *const Aarray[], 
		int lda,
		const CULIP_TYPE *const Barray[],
		int ldb, 
	...
	*(void**)(&cublas_lib_func) = CULiP_get_function_pointer(CULIP_CUBLAS_LIBRARY_NAME, CULIP_CUBLAS_ENV_NAME, __func__, &CULiP_cublas_lib_handle_cache);
	...
	if (profiling_flag) {
		...
		snprintf(profile_result.function_name, profile_result.function_name_length - 1, "%s-m%d-n%d-k%d-batchCount%d", __func__, m, n ,k, batchCount);
	...
	// Call the function
	const cublasStatus_t result = (*cublas_lib_func)(handle, transa, transb, m, n, k, alpha, Aarray, lda, Barray, ldb, beta, Carray, ldc, batchCount);
