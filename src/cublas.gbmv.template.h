cublasStatus_t CULIP_FUNC_NAME(cublasHandle_t handle, cublasOperation_t trans,
                               ...
                               int lda, const CULIP_TYPE *x, int incx,
  ...
  cublasStatus_t (*cublas_lib_func)(cublasHandle_t, cublasOperation_t, int, int,
                                    ...
                                    int, const CULIP_TYPE *, CULIP_TYPE *, int);
  *(void **)(&cublas_lib_func) = CULiP_get_function_pointer(
  ...
  if (profiling_flag) {
    ...
    snprintf(profile_result.function_name,
             profile_result.function_name_length - 1, "%s-%s-m%d-n%d-kl%d-ku%d",
             __func__, CULiP_get_cublasOperation_t_string(trans), m, n, kl, ku);
