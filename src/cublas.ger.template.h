cublasStatus_t CULIP_FUNC_NAME(cublasHandle_t handle, int m, int n,
                               const CULIP_TYPE *alpha, const CULIP_TYPE *x,
                               int incx, const CULIP_TYPE *y, int incy,
  ...
  cublasStatus_t (*cublas_lib_func)(cublasHandle_t, int, int,
                                    const CULIP_TYPE *, const CULIP_TYPE *, int,
                                    const CULIP_TYPE *, int, CULIP_TYPE *, int);
  *(void **)(&cublas_lib_func) = CULiP_get_function_pointer(__func__);
  ...
  if (profiling_flag) {
    ...
    snprintf(profile_result.function_name,
             profile_result.function_name_length - 1, "%s-m%d-n%d", __func__, m,
             n);
  ...
  const cublasStatus_t result =
      (*cublas_lib_func)(handle, m, n, alpha, x, incx, y, incy, A, lda);
