cublasStatus_t CULIP_FUNC_NAME(cublasHandle_t handle, cublasFillMode_t uplo,
                               int n, int k, const CULIP_TYPE *alpha,
  ...
  cublasStatus_t (*cublas_lib_func)(cublasHandle_t, cublasFillMode_t, int, int,
  ...
  *(void **)(&cublas_lib_func) = CULiP_get_function_pointer(
  ...
  if (profiling_flag) {
    ...
    snprintf(profile_result.function_name,
             profile_result.function_name_length - 1, "%s-%s-n%d-k%d", __func__,
             CULiP_get_cublasFillMode_t_string(uplo), n, k);
  ...
  // Call the function
  const cublasStatus_t result = (*cublas_lib_func)(handle, uplo, n, k, alpha, A,
