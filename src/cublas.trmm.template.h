cublasStatus_t CULIP_FUNC_NAME(cublasHandle_t handle, cublasSideMode_t side,
  ...
  cublasStatus_t (*cublas_lib_func)(
      ...
      cublasDiagType_t, int, int, const CULIP_TYPE *, const CULIP_TYPE *, int,
  ...
  *(void **)(&cublas_lib_func) = CULiP_get_function_pointer(
  ...
  if (profiling_flag) {
    ...
    snprintf(profile_result.function_name,
             profile_result.function_name_length - 1, "%s-%s-%s-%s-%s-m%d-n%d",
             __func__, CULiP_get_cublasSideMode_t_string(side),
             CULiP_get_cublasFillMode_t_string(uplo),
             CULiP_get_cublasOperation_t_string(trans),
             CULiP_get_cublasDiagType_t_string(diag), m, n);
